        ...
        public string LatestChangeForDisplay()
        {
            switch (Modified)
            {
                case null when !Created.HasValue:
                    return AppErrors.System_InfoNotAvailable;
                case null:
                    return Created.Value.ToStringOrEmpty("d MMM yyyy", AppErrors.System_InfoNotAvailable);
                default:
                    return Modified.Value.ToStringOrEmpty("d MMM yyyy", AppErrors.System_InfoNotAvailable);
            }
        }
    }
}
